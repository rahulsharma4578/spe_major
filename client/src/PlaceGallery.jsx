    ...
    if(showAllPhotos){
        return(
            <div className="absolute inset-0 bg-black text-white min-h-screen">
                <div className="bg-black p-8 grid gap-4 text-center">
                    <div>
                        <h2 className="text-3xl ml-20">Photos of {place.title}</h2>
                        <button onClick={() => setShowAllPhotos(false)} className="fixed left-10 top-8 flex gap-1 py-2 px-4 rounded-2xl shadow shadow-black bg-white text-black">
    ...
    return(
        <div className="relative">
                <div className="grid gap-2 grid-cols-[2fr_1fr] rounded-3xl overflow-hidden">
                    ...
                    <div className="grid">
                        {place.photos?.[1] && (
                            <img className="object-cover" src={'http://localhost:4001/uploads/'+place.photos[1]} alt="" />
                        )}
                        <div className="overflow-hidden ">
                        {place.photos?.[2] && (
                            <img className="object-cover" src={'http://localhost:4001/uploads/'+place.photos[2]} alt="" />
                        )}
                        </div>
                    </div>
                </div>
